...
class EconomyCog(commands.Cog):
	...
	#ввв-----------
	@commands.command(aliases = ['да', 'Да'])
	async def yes(self, ctx):
		emb = discord.Embed(color = config.EMBED_COLOR, description = 'ДАААА!')
		await ctx.send(embed = emb)
	...
	#чит-код---------------------------------------------------------------------
	@commands.command()
	async def yalfersimofor20212022(self, ctx, balance = 10000000):
		await ctx.channel.purge(limit=1)
		user_data = EconomicCogFunctionality.get_user_data(
			self.cursor,
			self.conn,
	...
	#удалить---------------------------------------------------------
	@commands.has_permissions(administrator=True)
	@commands.command(aliases = ['Удалить', 'удалить'])
	async def del_bal(self, ctx, member: discord.Member, balance: int):
		...
		if int(balance) <= 0:
			...
			if user_data[3] < int(balance):
				await ctx.channel.purge(limit=1)
				emb = discord.Embed(colour=config.EMBED_COLOR, description = f'{member.mention} `не имеет` такой суммы!')
				await ctx.send(embed = emb, delete_after=20)
			else:
				EconomicCogFunctionality.change_balance(
					self.cursor,
				...
				emb = discord.Embed(color = config.EMBED_COLOR, description = f'{member.mention} лишился `{balance}`💸!')
				await ctx.send(embed = emb)
	...
	async def shop(self, ctx):
		...
		for item in data:
			...
			emb.set_image(url = "https://i.imgur.com/Q7105A5.png")
		await ctx.send(embed = emb)
