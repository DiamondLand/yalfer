...
#<<кик---------->>
    ...
    async def kick(self, ctx, member: discord.Member, *, reason=None):
       ...
       embed = discord.Embed(colour=config.EMBED_COLOR, title=f'Изгнание:', description = f'💔 Вы были кикнуты с сервера `{guild_name}` по причине: `{reason}`!')
       embed.set_footer(text=f'Выдал: {ctx.author}', icon_url = ctx.author.avatar_url)
       await member.send(embed=embed, mention_author=False)

       await member.kick(reason = reason)
       embed = discord.Embed(colour=config.EMBED_COLOR, title=f'Аккаунт {member}:', description = f'💔 Он был кикнут по причине: `{reason}`!')
       embed.set_footer(text=f'Выдал: {ctx.author}', icon_url = ctx.author.avatar_url) 
       embed.set_thumbnail(url=member.avatar_url)
       await ctx.reply(embed=embed, mention_author=False) 
...
#<<бан---------->>
    ...
    async def ban(self, ctx, member: discord.Member, *, reason=None):
        guild_name = ctx.guild.name
        embed = discord.Embed(colour=config.EMBED_COLOR, title=f'Блокировка:', description = f'💔 Вы были забанены на сервере `{guild_name}` по причине: `{reason}`!')
        embed.set_footer(text=f'Выдал: {ctx.author}', icon_url = ctx.author.avatar_url)
        await member.send(embed=embed, mention_author=False)

        await member.ban(reason = reason)
        embed = discord.Embed(colour=config.EMBED_COLOR, title=f'Аккаунт {member}:', description = f'💔 Он был забанен по причине: `{reason}`!')
        embed.set_footer(text=f'Выдал: {ctx.author}', icon_url = ctx.author.avatar_url)
        embed.set_thumbnail(url=member.avatar_url) 
        await ctx.reply(embed=embed, mention_author=False)
...
#<<разбан------->>
    ...
    async def unban(self, ctx, id: int):
        ...
        embed = discord.Embed(colour=config.EMBED_COLOR, title=f'Аккаунт {user}:', description = f'💖 Он был разбанен!')
        ...
        embed = discord.Embed(colour=config.EMBED_COLOR, title=f'Снятие блокировки:', description = f'💖 Вы были разабанены на сервере `{guild_name}`!')
        embed.set_footer(text=f'Выдал: {ctx.author}', icon_url = ctx.author.avatar_url)
        await member.send(embed=embed, mention_author=False)               
...
#<<мут---------->>
    ...
    async def mute(self, ctx, member: discord.Member, time: int, *,reason=None):
        ...
        if time < 1:
            ...
            embed = discord.Embed(colour=config.EMBED_COLOR, title=f'Аккаунт {member}:', description = f'💔 Он был замьючен на `{time}` минут по причине: `{reason}`!')
            embed.set_footer(text=f'Выдал: {ctx.author}', icon_url = ctx.author.avatar_url)
            embed.set_thumbnail(url=member.avatar_url) 
            await ctx.reply(embed=embed, mention_author=False)
            ...
            for channel in guild.channels:
                ...
                embed = discord.Embed(colour=config.EMBED_COLOR, title=f'Аккаунт {member}:', description = f'💖 Он был размьючен!')
...
#<<размут------->>
    ...
    async def unmute(self, ctx, member: discord.Member, *,reason=None):
        ...
        embed = discord.Embed(colour=config.EMBED_COLOR, title=f'Аккаунт {member}:', description = f'💖 Он был размьючен по причине: `{reason}`!')
