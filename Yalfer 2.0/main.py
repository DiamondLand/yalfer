...
async def on_command_error(ctx, error):
    ...
    if str(error) == "Command raised an exception: NotFound: 404 Not Found (error code: 10008): Unknown Message":
        return
    if str(error) == "Command raised an exception: AttributeError: 'NoneType' object has no attribute 'is_playing'":
        await error_send.send_error(ctx, "Активный поток музыки не найден")
        return
