...
class LevelSystem(commands.Cog):
    ...
    async def on_message(self, msg):
        ...
        if new_server_level != current_server_level:
           embed = discord.Embed(description = f"{msg_author.mention} получает новый уровень! `{new_server_level}`", color=config.EMBED_COLOR)
    ...
    async def level(self, ctx, member: discord.Member = None):
        ...
        embed.add_field(name = "Осталось до следующего уровня: ", value = ":green_square:" * int(done / 200) + ":yellow_square:" * int(not_done / 200) + f" ({done / 20} %)", inline = False)
