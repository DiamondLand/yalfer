...
class EconomyCog(commands.Cog):
    ...
    async def to_bank(self, ctx, balance: int):
        ...
        if user_data[3] < int(balance):
            ...
            if user_data[2] >= 999999999999999999:
                ...
                emb = discord.Embed(color = config.EMBED_COLOR, title=f'💰 Перевод в банк:', description = f'> Переведено в банк: `{balance}` 💸!')
    ...
    async def from_bank(self, ctx, balance: int):
        ...
        if user_data[2] < int(balance):
            ...
            if int(balance) + users_balance > 999999999999999999:
                ...
                self.conn.commit()
                emb = discord.Embed(color = config.EMBED_COLOR, title=f'💰 Перевод из банка:', description = f'> Переведено в наличку: `{balance}` 💸!')
                emb.set_footer(text = ctx.author.name, icon_url = ctx.author.avatar_url)
                await ctx.reply(embed = emb, mention_author=False)
    ...
    async def fond(self, ctx, balance: int):
        ...
        if int(balance) <= 0:
            ...
            if user_data[3] < int(balance):
                ...
                emb = discord.Embed(color = config.EMBED_COLOR, title=f'🎭 Пожертвование:', description = f'> Изъято: `{balance}` 💸 у {ctx.author.mention}!')
                emb.set_footer(text = 'Денежные средства не могу быть возвращены!')
                await ctx.reply(embed = emb, mention_author=False)
    ...
    async def add_shop_item(self, ctx, role: discord.Role, prise: int):
        ...
        if int(prise) <= 0:
            ...
            self.conn.commit()
            emb = discord.Embed(color = config.EMBED_COLOR, title=f'🎪 Магазин:', description = f'> Добавлено: {role.mention} • `{prise}` 💸!')
            emb.set_footer(text = ctx.author.name, icon_url = ctx.author.avatar_url)
            await ctx.reply(embed = emb, mention_author=False)
    ...
    async def del_shop_item(self, ctx, role: discord.Role):
        ...
        self.conn.commit()
        emb = discord.Embed(color = config.EMBED_COLOR, title=f'🎪 Магазин:', description = f'> Удалено: {role.mention}!')
        emb.set_footer(text = ctx.author.name, icon_url = ctx.author.avatar_url)
        await ctx.reply(embed = emb, mention_author=False)
    ...
    async def shop(self, ctx):
        ...
        data.reverse()
        emb = discord.Embed(color = config.EMBED_COLOR, title="🎪 Магазин:")
        for item in data:
    ...
    async def buy(self, ctx, role: discord.Role):
        ...
        if not role_exists:
            ...
            if user_data[3] < int(balance):
                ...
                emb = discord.Embed(color = config.EMBED_COLOR, title=f'💵 Покупка:', description = f'> Куплено: {role.mention}!')
                emb.set_footer(text = ctx.author.name, icon_url = ctx.author.avatar_url)
                await ctx.reply(embed = emb, mention_author=False)
