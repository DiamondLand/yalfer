...
class EconomyCog(commands.Cog):
	...
	async def set_wallet(self, ctx, member: discord.Member, balance: int):
		...
		self.conn.commit()
		emb = discord.Embed(colour = discord.Color.green(), description = f':white_check_mark: Баланс {member.mention} установлен на `{balance}`💸!')
	...
	async def add_bal(self, ctx, member: discord.Member, balance: int):
		...
		emb = discord.Embed(colour = discord.Color.green(), description = f':white_check_mark: Успешно добавлено `{balance}`💸!')
		await ctx.send(embed = emb)

	@commands.command(aliases = ['Бонус', 'бонус'])
	@commands.cooldown(1, 3600, commands.BucketType.user)
	async def bonus(self, ctx, balance = 15000):
		"""
		:param ctx:
		:param member:
		:param balance:
	...
	async def send_gift(self, ctx, member: discord.Member, cash: int):
		...
		if int(cash) > users_balance:
			...
			emb = discord.Embed(colour = discord.Color.green(), description = f'{author.mention}, вы передали {member.mention} `{cash}`💸!')
			await ctx.send(embed = emb)
	...
	async def del_bal(self, ctx, member: discord.Member, balance: int):
		...
		if user_data[3] < int(balance):
			...
			emb = discord.Embed(colour = discord.Color.green(), description = f':white_check_mark: Успешно удалено `{balance}`💸!')
			await ctx.send(embed = emb)
