...
async def on_ready():
    ...
    await bot.change_presence(status=discord.Status.online, activity=discord.Activity(type=discord.ActivityType.playing, name=f"+хелп"))
...
for folder in os.listdir("cogs"):
    for file in os.listdir(f"cogs/{folder}"):
        if file.endswith(".py") and file.startswith("COG_"):
            ...
            logger.info(f"import \"F:\Yalfer\Yalfer 2.3\cogs/{folder}/{file}\"")
